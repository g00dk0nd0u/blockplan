...
test("equal-strength Memory ordering is code-unit deterministic across browser locales", async ({ browser }) => {
  const memoryIds = ["memory-é", "memory-a", "memory-あ", "memory-Z"];
  const orders = [];
  for (const locale of ["en-US", "ja-JP"]) {
    ...
    await page.goto(appUrl);
    const seeded = await seedReviewedVariant(page);
    orders.push(await page.evaluate(({ snapshotId, ids }) => {
      ids.forEach((memoryId) => {
        window.BlockPlanAPI.proposeMemoryFromReviews({
          memoryId,
          scope: "project",
          type: "soft-preference",
          statement: memoryId,
        ...
        window.BlockPlanAPI.approveMemory(memoryId);
      });
      return window.BlockPlanAPI.getRelevantMemory({ requirementsSnapshotId: snapshotId }).map((item) => item.memoryId);
    }, { ...seeded, ids: memoryIds }));
    await context.close();
  }
  expect(orders).toEqual([
    ["memory-Z", "memory-a", "memory-é", "memory-あ"],
    ["memory-Z", "memory-a", "memory-é", "memory-あ"]
  ]);
});
