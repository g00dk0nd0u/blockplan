...
(function installDesignMemory() {
  ...
  function requireValidState(state, reviewState) {
    ...
    state.items.forEach((item) => {
      ...
      if (typeof item.createdAt !== "string" || !item.createdAt.trim()) throw new Error(`Memory createdAt is required: ${item.memoryId}`);
      if (typeof item.updatedAt !== "string" || !item.updatedAt.trim()) throw new Error(`Memory updatedAt is required: ${item.memoryId}`);
    });
    return state;
  }
  ...
  function contextValues(context) {
    ...
    if (snapshot && snapshot.requirements) {
      (snapshot.requirements.bubbles || []).forEach((bubble) => { if (bubble.type) bubbleTypes.add(String(bubble.type)); });
      (snapshot.requirements.connectors || []).forEach((connector) => { if (connector.relationType) relationTypes.add(String(connector.relationType)); });
    }
    return {
